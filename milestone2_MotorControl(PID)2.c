...
void PIDControl(int hgt_percent, double dt)
{
	...
	static signed long yawInt;
	static signed long altInt;
	static signed long yawDer;
	...
	// PID gain values
	kpAlt = 0.2; // altitude proportional gain, decreases the response to system disturbances
	kiAlt = 0.003; // altitude integral gain, eliminates steady state error
	kdAlt = 0.1; // altitude derivative gain, accounts for overshoot produced by ki and kp

	kpYaw = 0.6; // yaw proportional gain, decreases the response to system disturbances
	kiYaw = 0.009; // yaw integral gain, eliminates steady state error
	kdYaw = 2.5; // yaw derivative gain, accounts for overshoot produced by ki and kp

	altErrPrev = altErr;
	yawErrPrev = yawErr;

	altErr = desiredHeight - hgt_percent; // positive if up
	yawErr = desiredYaw - yaw; // positive if cw
