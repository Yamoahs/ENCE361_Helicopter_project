...
void PIDControl(int currentHeight, int desiredHeight, int currentYaw, int desiredYaw, double dt, int tail_duty, int main_duty)
{
	...
	altPrevError = yawError;
	yawPrevError = yawError;

	yawError = desiredHeight - currentHeight;
	yawError = desiredYaw - currentYaw;


	altProportion = error * Kp;
	altIntergral = integralControl(yawError, altKi, dt);
	altDerivative = derivativeControl(yawError, altPrevError, altKd, dt);
...
int altPIDControl(int currentHeight, int initialRead, int desiredHeight, double dt)
{
	float altKp = 0.05;
	//float altKi = 0.005;
	//float altKd = 0.05;
	double altProportion = 0;
	//static double altIntergral = 0;
	//static double altDerivative = 0;
	//static double altPrevError = 0;
	static signed long altError = 0;
	static double altControl = 0;

	altError = desiredHeight -  currentHeight;

	altProportion = altError * altKp;
	//altIntergral += altError * dt;
	//altDerivative = (altError - altPrevError) / dt;


	//altControl = 10 + altProportion + (altIntergral * altKi) + (altDerivative * altKd);
	//altPrevError = altError;
	altControl = 10 + altProportion;
...
int yawPIDControl(int yaw, int desiredyaw, double dt)
{
	float yawKp = 0.5;
	float yawKi = 0.005;
	float yawKd = 2.0;
	double yawProportion;
	static double yawIntergral;
	static double yawDerivative;
	static double yawPrevError;
	static signed long yawError;
	static double yawControl = 0;
	...
	yawControl = 10 + yawProportion + (yawIntergral * yawKi) + (yawDerivative * yawKd);
	yawPrevError = yawError;
...
//*****************************************************************************
void displayInfo(int newHght, int inital, int height, int degrees, int main, int tail)
{
	char string[40];
...
int main(void)
{
	...
	while (1)
	{
		...
		main_duty = altPIDControl(hgt_percent, initialRead, desiredHeight, dt);
		tail_duty = yawPIDControl(yaw, desiredYaw, dt);
		PWMPulseWidthSet (PWM_BASE, PWM_OUT_1, period * main_duty / 100);
		PWMPulseWidthSet (PWM_BASE, PWM_OUT_4, period * tail_duty / 100);
		/*char string[40];
		if ((g_ulSampCnt % 25) == 0){
		...
		displayInfo(newHght, (int)initialRead, hgt_percent, degrees, main_duty, tail_duty);
	}
}
